...
Overall, batch size primarily affects inference quality rather than goodput or resource utilization. Resource usage patterns are governed by kernel execution, while real-world performance depends on the scheduling strategy's ability to mitigate interference and preserve responsiveness. {\roomie}'s proactive colocation demonstrates its strongest advantage at moderate batch sizes, underscoring that interference-aware design—not raw batching for utilization—is the critical factor under high load.
